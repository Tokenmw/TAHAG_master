...
class unit_gcn(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.adaptive:
            ...
            A1 = A + A1 * self.alpha
            A2 = x
            y = self.conv_d(torch.matmul(A2, A1))
        else:
...
class Model(nn.Module):
    def __init__(self, in_feats, num_class = 4, adj = None, drop_out = 0, adaptive = True, attention = True):
    ...
    def forward(self, x):
        feats = self.sharedNet(x)
        # classifier.
        feats = self.drop_out(feats)
        cls_out = self.fc(feats)
